...
type Describer = (config: AiConfig, imageDataUrl: string, prompt: string, signal: AbortSignal) => Promise<string>
...
async function describeAnthropic(config: AiConfig, imageDataUrl: string, prompt: string, signal: AbortSignal): Promise<string> {
      ...
      messages: [
        {
          role: 'user',
          content: [
            { type: 'image', source: { type: 'base64', media_type: parts.mime, data: parts.base64 } },
            { type: 'text', text: prompt },
          ],
        },
      ],
...
async function describeGemini(config: AiConfig, imageDataUrl: string, prompt: string, signal: AbortSignal): Promise<string> {
  const parts = splitDataUrl(imageDataUrl)
  if (!parts) throw new Error('La imagen no tiene un formato reconocible.')
  ...
  const json = (await postJson(
    endpoint,
    { 'Content-Type': 'application/json' },
    { contents: [{ role: 'user', parts: [{ text: prompt }, { inline_data: { mime_type: parts.mime, data: parts.base64 } }] }] },
    signal,
    false,
...
export async function describeImage(config: AiConfig, imageDataUrl: string, prompt: string, signal: AbortSignal): Promise<string> {

...
describe('streamChat', () => {
  ...
  it('OpenAI: accumulates delta.content tokens and stops at [DONE]', async () => {
    ...
    const tokens: string[] = []
    await streamChat(baseConfig('OpenAI'), messages, (t) => tokens.push(t), new AbortController().signal)
  ...
  it('OpenAI: silently ignores a malformed (non-JSON) chunk instead of throwing', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(sseResponse(['not json at all', JSON.stringify({ choices: [{ delta: { content: 'ok' } }] }) ]))
    vi.stubGlobal('fetch', fetchMock)
  ...
  it('Mistral: uses the same OpenAI-compatible parser', async () => {
    const fetchMock = vi.fn().mockResolvedValue(sseResponse([JSON.stringify({ choices: [{ delta: { content: 'bonjour' } }] })]))
    vi.stubGlobal('fetch', fetchMock)

    const tokens: string[] = []
    await streamChat(baseConfig('Mistral'), messages, (t) => tokens.push(t), new AbortController().signal)
    expect(tokens.join('')).toBe('bonjour')
  })

  it('Custom: normalizes the configured base URL to /v1/chat/completions', async () => {
    const fetchMock = vi.fn().mockResolvedValue(sseResponse([JSON.stringify({ choices: [{ delta: { content: 'x' } }] })]))
    vi.stubGlobal('fetch', fetchMock)
    ...
    expect(fetchMock).toHaveBeenCalledWith('http://127.0.0.1:1234/v1/chat/completions', expect.anything())
  })
  ...
  it('Anthropic: reads text from content_block_delta events and ignores other event types', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      sseResponse([
        ...
        JSON.stringify({ type: 'content_block_delta', delta: { type: 'text_delta', text: 'Hola' } }),
        JSON.stringify({ type: 'content_block_delta', delta: { type: 'text_delta', text: ' mundo' } }),
    ...
    const tokens: string[] = []
    await streamChat(baseConfig('Anthropic'), messages, (t) => tokens.push(t), new AbortController().signal)
    expect(tokens.join('')).toBe('Hola mundo')
    expect(fetchMock).toHaveBeenCalledWith(
  ...
  it('Google Gemini: reads text from candidates[0].content.parts[0].text', async () => {
    ...
    const tokens: string[] = []
    await streamChat(baseConfig('Google Gemini'), messages, (t) => tokens.push(t), new AbortController().signal)
    expect(tokens.join('')).toBe('Hola mundo')
    expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining('generativelanguage.googleapis.com'), expect.anything())
  })
  ...
  it('Cohere: reads text from content-delta events only', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      sseResponse([
        ...
        JSON.stringify({ type: 'content-delta', delta: { message: { content: { text: 'Hola mundo' } } } }),
    ...
    const tokens: string[] = []
    await streamChat(baseConfig('Cohere'), messages, (t) => tokens.push(t), new AbortController().signal)
    expect(tokens.join('')).toBe('Hola mundo')
  })

  it('throws the server-provided error message when the response is not ok', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(errorResponse(401, { error: { message: 'Invalid API key' } })))
    await expect(
  ...
  it('re-throws AbortError from fetch without rewriting it into a generic connection error', async () => {
    ...
    await expect(streamChat(baseConfig('OpenAI'), messages, () => {}, new AbortController().signal)).rejects.toBe(
      abortError,
    )
  })

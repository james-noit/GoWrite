...
import { describeImage, supportsImageDescription } from '../describeImage'
...
describe('describeImage', () => {
  ...
  it('OpenAI-compatible providers: reads choices[0].message.content', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(jsonResponse({ choices: [{ message: { content: 'A cat.' } }] })))
    const result = await describeImage(baseConfig('OpenAI'), PNG_DATA_URL, 'describe', new AbortController().signal)
  ...
  it('Anthropic: sends the image as a base64 source block and reads content[0].text', async () => {
    ...
    const result = await describeImage(baseConfig('Anthropic'), PNG_DATA_URL, 'describe', new AbortController().signal)
    expect(result).toBe('A cat.')

    const call = fetchMock.mock.calls[0]
    const body = JSON.parse(call[1].body)
    expect(body.messages[0].content[0]).toMatchObject({ type: 'image', source: { type: 'base64', media_type: 'image/png', data: 'QUJD' } })
  })
  ...
  it('Anthropic: rejects a non-data-URL image before making any request', async () => {
    ...
    vi.stubGlobal('fetch', fetchMock)
    await expect(
      describeImage(baseConfig('Anthropic'), 'https://example.com/a.png', 'describe', new AbortController().signal),
  ...
  it('Google Gemini: reads candidates[0].content.parts[0].text', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ candidates: [{ content: { parts: [{ text: 'A cat.' }] } }] })),
    )
    const result = await describeImage(baseConfig('Google Gemini'), PNG_DATA_URL, 'describe', new AbortController().signal)
    expect(result).toBe('A cat.')
  })

  it('Mistral: uses the OpenAI-compatible shape against the Mistral endpoint', async () => {
    const fetchMock = vi.fn().mockResolvedValue(jsonResponse({ choices: [{ message: { content: 'A cat.' } }] }))
    vi.stubGlobal('fetch', fetchMock)
    await describeImage(baseConfig('Mistral'), PNG_DATA_URL, 'describe', new AbortController().signal)
    expect(fetchMock).toHaveBeenCalledWith('https://api.mistral.ai/v1/chat/completions', expect.anything())
  })

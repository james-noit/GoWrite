...
async function generateGeminiImage(config: AiConfig, prompt: string, signal: AbortSignal): Promise<string> {
...
const IMAGE_GENERATORS: Partial<Record<AiProviderId, ImageGenerator>> = {
  OpenAI: (config, prompt, signal) =>
    generateOpenAiCompatibleImage('https://api.openai.com/v1/images/generations', config.apiKey, 'dall-e-3', prompt, signal, false),
...
export async function generateImage(config: AiConfig, prompt: string, signal: AbortSignal): Promise<string> {

...
describe('testConnection', () => {
  ...
  it('OpenAI-style response ({data:[{id}]}) is parsed and sorted alphabetically', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(jsonResponse({ data: [{ id: 'gpt-4o' }, { id: 'gpt-3.5-turbo' }] })),
    );
  ...
  it('Gemini/Cohere-style response ({models:[{name}]}) strips the "models/" prefix', async () => {
    ...
    expect(await testConnection(baseConfig('Google Gemini'))).toEqual([
      'gemini-1.5-flash',
      'gemini-1.5-pro',
    ]);
  });
  ...
  it('Custom provider: builds the models URL from the configured base endpoint', async () => {
    ...
    await testConnection(
      baseConfig('Custom', { customEndpoint: 'http://127.0.0.1:1234/v1/chat/completions' }),
    );

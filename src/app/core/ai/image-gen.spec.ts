...
import { generateImage, supportsImageGeneration } from './image-gen';
...
describe('generateImage', () => {
  ...
  it('OpenAI: returns a data URL built from the b64_json response', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ data: [{ b64_json: 'QUJD' }] })),
    );
  ...
  it('OpenAI: throws when the provider returns no image data', async () => {
    ...
    await expect(
      generateImage(baseConfig('OpenAI'), 'a cat', new AbortController().signal),
    ).rejects.toThrow('no devolvió ninguna imagen');
  });

  it('Google Gemini: returns a data URL built from bytesBase64Encoded', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ predictions: [{ bytesBase64Encoded: 'QUJD' }] }));
  ...
  it('rejects unsupported providers (Anthropic) with a clear "not supported" error', async () => {
    await expect(
      generateImage(baseConfig('Anthropic'), 'a cat', new AbortController().signal),
    ).rejects.toThrow('no admite generación de imágenes');
  });

  it('throws the server error message on a non-ok response', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(jsonResponse({ error: { message: 'Quota exceeded' } }, 429)),
    );
    await expect(
      generateImage(baseConfig('OpenAI'), 'a cat', new AbortController().signal),

...
import { collectImageSizes, decodeDataUrl, extensionForMime } from './image-utils';
...
function blockNodeToHtml(
  ...
  if (local === 'h') {
    const level = Math.min(
      3,
      Math.max(1, Number(node.getAttributeNS(NS.text, 'outline-level') ?? '1')),
    );
    ...
    const rowsHtml = rows
      .map((rowEl, rowIndex) => {
        ...
        const cellsHtml = Array.from(rowEl.children)
          ...
          .map((cellEl) => {
            const blocks = Array.from(cellEl.children).filter(
              (c) => c.localName === 'p' || c.localName === 'h',
            );
            const innerHtml = renderBlocks(blocks, charStyles, listKinds, paraAlign) || '<p></p>';
            const isHeaderCell =
              isHeaderRow ||
              blocks.some((b) => b.getAttributeNS(NS.text, 'style-name') === 'TableHeader');
...
function buildManifestXml(images: OdtImageEntry[]): string {
  const imageEntries = images
    .map(
      (img) =>
        `  <manifest:file-entry manifest:full-path="Pictures/${img.fileName}" manifest:media-type="${img.mime}"/>`,
    )
    .join('\n');
...
function buildContentXml(json: JSONContent, images: Map<string, OdtImageEntry>): string {
  ...
  function tableToOdt(node: JSONContent): string {
    ...
    const columnsXml =
      columnCount > 0 ? `<table:table-column table:number-columns-repeated="${columnCount}"/>` : '';
  ...
  function blockToOdt(node: JSONContent, paragraphStyle: string): string {
    switch (node.type) {
      case 'paragraph': {
        const styleName = styleForAlign(
          paragraphStyle,
          node.attrs?.['textAlign'] as string | undefined,
        );
...
async function prepareOdtImages(
  json: JSONContent,
): Promise<{ images: Map<string, OdtImageEntry>; files: Map<string, Uint8Array> }> {
  ...
  for (const src of srcs) {
    ...
    images.set(src, {
      fileName,
      mime: decoded.mime,
      widthCm: width * PX_TO_CM,
      heightCm: height * PX_TO_CM,
    });
